...
class EntitySpringRestResourceFile extends XtendTemplateFile<Entity> {
	...
	override public getDocumentation() '''The Spring REST controller for managing entity «element?.name?.toFirstUpper»'''

	override public getTemplateName() '''«this.class.name»'''
	
	override public ProjectType getProject() { ProjectType.REST_SERVER }

	override public getFileName() '''«element?.name?.toFirstUpper»RestResource'''

	override public getPath() '''«super.path»/src/main/java/«stubb?.packageName?.toPath»/rest/resource/'''
